        ...
        loaiSachHolder.imgDel.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (theLoaiDAO.deleteTheLoai(loaiSach.getMaTheLoai()) > 0) {
                    Toast.makeText(context, "Xóa thành công thể loại sách !", Toast.LENGTH_SHORT).show();
                    listLoaiSach.remove(i);
                    notifyDataSetChanged();
                } else {
                    Toast.makeText(context, "Xóa không thành công thể loại sách !", Toast.LENGTH_SHORT).show();
                }
